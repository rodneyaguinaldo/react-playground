...
function reducer(state, action) {
  ...
  switch (action.type) {
    case "openAccount":
      return { ...state, isActive: true, balance: INITIAL_BALANCE };
    case "deposit":
      return { ...state, balance: state.balance + DEPOSIT_AMOUNT };
    case "withdraw":
      return {
        ...state,
        balance: state.balance > 0 ? state.balance - WITHDRAW_AMOUNT : 0,
      };
    case "requestLoan":
      return {
        ...state,
        hasLoan: true,
        balance: state.hasLoan ? state.balance : state.balance + REQUEST_LOAN,
        loan: REQUEST_LOAN,
      };
    case "payLoan":
      return {
        ...state,
        hasLoan: false,
        balance: state.hasLoan && state.balance - REQUEST_LOAN,
        loan: state.hasLoan && 0,
      };
    case "closeAccount":
      return {
        ...state,
